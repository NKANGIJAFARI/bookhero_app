...
import createUser from '../../AppFunctions/Auth/createUser';
...
const CreateUserScreen = () => {
  ...
  const onSubmit = async () => {
    ...
    if (response.error === 'TOO_LONG') {
      setPhoneNumber({
        value: phoneNumber.value,
        error: 'Phone Number too long. Please enter a valid number',
      });
      dispatch(
        setLoading({
          isLoading: false,
    ...
    return (
      <TouchableOpacity activeOpacity={0.9} onPress={onSubmit}>
        <LinearGradient
          ...
          <Text style={{ ...Fonts.white16Regular }}>Create New User</Text>
        </LinearGradient>
      </TouchableOpacity>
    );
  };
